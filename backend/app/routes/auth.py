...
async def login(request: Request, data: LoginData, db: Session = Depends(get_db)):
    try:
        if not validate_email(data.email): raise HTTPException(status_code=400, detail="Invalid email format")
        user = db.query(User).filter(User.email == data.email).first()
        if not user or not verify_password(data.password, user.password): raise HTTPException(status_code=401, detail="Invalid email or password")
        clinic_name = None
        if user.clinic_id:
        ...
        return {"access_token": access_token, "token_type": "bearer", "user": {"id": user.id, "name": user.name, "email": user.email, "phone": user.phone, "user_type": user.user_type, "clinic_id": user.clinic_id, "clinic_name": clinic_name, "age": user.age, "gender": user.gender, "is_active": bool(user.is_active), "created_at": str(user.created_at)}}
...
async def register_patient(request: Request, data: dict, db: Session = Depends(get_db)):
    try:
        ...
        if db.query(User).filter(User.phone == data["phone"]).first(): raise HTTPException(status_code=400, detail="Phone number already registered")
        hashed_password = hash_password(data["password"])
        age = int(data["age"]) if data.get("age") and str(data["age"]).isdigit() else None
        user = User(name=data["name"].strip(), email=data["email"].strip().lower(), phone=data["phone"].strip(), password=hashed_password, user_type="patient", age=age, gender=data.get("gender", "").strip() or None)
...
async def register_clinic(request: Request, data: dict, db: Session = Depends(get_db)):
    try:
        ...
        if db.query(User).filter(User.email == data["email"]).first(): raise HTTPException(status_code=400, detail="Email already registered")
        hashed_password = hash_password(data["password"])
        user = User(name=data["name"].strip(), email=data["email"].strip().lower(), phone=data["phone"].strip(), password=hashed_password, user_type="clinic", clinic_id=data["clinic_id"])
        db.add(user); db.commit(); db.refresh(user)
        return {"id": user.id, "name": user.name, "email": user.email}
    except HTTPException: raise
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
async def forgot_password(request: Request, data: dict, db: Session = Depends(get_db)):
    try:
        email = data.get("email", "").strip().lower(); phone = data.get("phone", "").strip()
...
async def reset_password(data: dict, db: Session = Depends(get_db)):
    try:
        email = data.get("email", "").strip().lower(); code = data.get("code", "").strip(); new_password = data.get("new_password", "")
...
async def change_password(data: dict, db: Session = Depends(get_db)):
    try:
        ...
        user = db.query(User).filter(User.id == user_id).first()
        if not user: raise HTTPException(status_code=404, detail="User not found")
        if not verify_password(current_password, user.password): raise HTTPException(status_code=400, detail="Current password is incorrect")
        if not validate_password(new_password): raise HTTPException(status_code=400, detail="Password must be at least 6 characters")
        user.password = hash_password(new_password); db.commit()
